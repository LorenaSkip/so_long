...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/23 11:42:32 by lnitu             #+#    #+#             */
/*   Updated: 2023/04/13 11:03:53 by lnitu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../so_long.h"

void	put_wall(t_tile tile, t_game game, t_vector pos);

static void	draw_image(t_tile tile, t_game game, t_vector pos)
{
	if (tile.type == WALL)
		put_wall(tile, game, pos);
	else if (tile.type == EXIT)
	{
		...
		mlx_put_image_to_window(game.mlx, game.window,
			game.pokeball_imgs.current_img, pos.x, pos.y);
	else if (tile.type == PLAYER)
		mlx_put_image_to_window(game.mlx, game.window,
			game.player.current_img, pos.x, pos.y);
	else if (tile.type == GASTLY)
		mlx_put_image_to_window(game.mlx, game.window,
			game.gastly_imgs.basic_current, pos.x, pos.y);
	else if (tile.type == FOLLOWER)
		mlx_put_image_to_window(game.mlx, game.window,
			game.gastly_imgs.follow_current, pos.x, pos.y);
}
